...
tegrabct_values = { '--bct':None, '--list':'bct_list.xml', '--signed_list':'bct_list_signed.xml', '--mb1_bct':None, '--mb1_cold_boot_bct':None, '--membct_cold_boot': None, '--membct_rcm' : None, '--rcm_bct': None }
...
def tegraflash_secureflash(args):
    ...
    if tegrabct_values['--rcm_bct'] is not None:
...
def tegraflash_update_cfg_file(signed_files, cfg_file, output_dir, chip=None):
    ...
    if not values['--tegraflash_v2'] and int(values['--chip_major'], 0) >= 2:
        ...
        for nvc_part in nvc_parts:
            ...
            if values['--encrypt_key'] is not None:
                command = exec_file('tegrasign')
                command.extend(['--file', nvc_file, '--key', values['--encrypt_key'][0]])
                run_command(command)
            ...
            command = exec_file('tegrasign')
            command.extend(['--file', nvc_file, '--sha', 'sha256'])
            run_command(command)
        ...
        if values['--encrypt_key'] is not None:
            ...
            for wb0_part in wb0_parts:
                ...
                command = exec_file('tegrasign')
                command.extend(['--file', wb0_file, '--key', values['--encrypt_key'][0], '--offset', '816'])
                run_command(command)
...
def tegraflash_generate_mem_bct(is_cold_boot_mb1_bct):
    ...
    if bool(is_cold_boot_mb1_bct) == True:
        ...
        if os.path.isfile('chip_info.bin'):
            with open('chip_info.bin', 'rb') as f:
